...
switch ($error_reporting){
    ...
    case 2: error_reporting(E_ALL ^ E_DEPRECATED); @ini_set("display_errors",1); break;
}
...
function frame2(){
    ...
    <!--
        ...
        function set_fm_current_root(arg){
            document.location.href='".addslashes($fm_path_info["basename"])."?frame=2&set_fm_current_root='+encodeURIComponent(arg);
        }
...
function dir_list_form() {
    ...
    if ($opdir = @opendir(fs_encode($current_dir))) {
        ...
        function download(arg){
            parent.frame1.location.href='".addslashes($fm_path_info["basename"])."?action=3&current_dir=".addslashes($current_dir)."&filename='+encodeURIComponent(arg);
        }
        ...
        function execute_file(arg){
            if(arg.length>0){
                if(confirm('".et('ConfExec')." \\' '+arg+' \\' ?')) {
                    openModalWindow('".addslashes($fm_path_info["basename"])."?action=11&current_dir=".addslashes($current_dir)."&filename='+encodeURIComponent(arg),'".et('Exec')." '+encodeURIComponent(arg),800,600);
                }
            }
        }
        function edit_file_form(arg){
            openModalWindow('".addslashes($fm_path_info["basename"])."?action=7&current_dir=".addslashes($current_dir)."&filename='+encodeURIComponent(arg),'".et('Edit')." '+encodeURIComponent(arg),1024,768);
        }
        ...
        function view_form(arg){
            if(confirm('".uppercase(et('View'))." \\' '+arg+' \\' ?')) openModalWindow('".addslashes($fm_path_info["basename"])."?action=4&current_dir=".addslashes($current_dir)."&filename='+encodeURIComponent(arg),'".et("View")." '+encodeURIComponent(arg),800,600);
        }
        function rename(arg){
            var nome = '';
            if (nome = prompt('".uppercase(et('Ren'))." \\' '+arg+' \\' ".et('To')." ...')) document.location.href='".addslashes($fm_path_info["basename"])."?frame=3&action=3&current_dir=".addslashes($current_dir)."&old_name='+encodeURIComponent(arg)+'&new_name='+encodeURIComponent(nome);
        }
...
function portscan_form(){
    ...
    switch ($portscan_action){
        case 1:
            ...
            echo "<br>\n";
            echo "Note: Maybe the server does not allow local network access using PHP sockets.<br>Good, this was major security problem on older PHP versions.<br>\n";
...
function et($tag){
    ...
    $ru['Version']='Версия';
    $ru['DocRoot']='Корневая папка';
    $ru['FLRoot']='Корневая папка файлового менеджера';
    $ru['Name']='Имя';
    $ru['And']='и';
    $ru['Enter']='Войти';
    $ru['Send']='Отправить';
    ...
    $ru['Dir_s']='Папка/и';
    $ru['To']='в';
    $ru['Destination']='Конечная папка';
    $ru['Configurations']='Конфигурация';
    $ru['JSError']='Ошибка JavaScript';
    $ru['NoSel']='Нет выбранных элементов';
    $ru['SelDir']='Выберите папку назначения в левом дереве';
    $ru['TypeDir']='Введите имя папки';
    $ru['TypeArq']='Введите имя файла';
    $ru['TypeCmd']='Введите команду';
    $ru['TypeArqComp']='Введите имя и расширение файла.\\nРасширение определит тип сжатия.\\n Пример: \\n nome.zip \\n nome.tar \\n nome.bzip \\n nome.gzip ';
    $ru['RemSel']='Удалить выбранные элементы';
    $ru['NoDestDir']='Не выбрана папка назначения';
    $ru['DestEqOrig']='Исходные и конечные папки равны';
    $ru['InvalidDest']='Конечная папка недействительна';
    $ru['NoNewPerm']='Новые разрешения не установлены';
    $ru['CopyTo']='Копировать в';
    $ru['MoveTo']='Переместить в';
    $ru['AlterPermTo']='Измененить разрешения на';
    $ru['ConfExec']='Подтвердить ВЫПОЛНЕНИЕ';
    $ru['ConfRem']='Подтвердить УДАЛЕНИЕ';
    $ru['EmptyDir']='Пустая папка';
    $ru['IOError']='Ошибка I/O';
    $ru['FileMan']='Файловый менеджер';
    $ru['TypePass']='Введите пароль';
    $ru['InvPass']='Неверный пароль';
    $ru['ReadDenied']='Доступ запрещен';
    $ru['FileNotFound']='Файл не найден';
    $ru['AutoClose']='Закрыть после окончания';
    $ru['OutDocRoot']='Файлы за пределами DOCUMENT_ROOT';
    $ru['NoCmd']='Ошибка: Команда не поддерживается';
    ...
    $ru['FileDirExists']='Файл или папка уже существует';
    $ru['NoPhpinfo']='Функция PHPInfo отключена';
    $ru['NoReturn']='Нет возврата';
    $ru['FileSent']='Файл отправлен';
    $ru['SpaceLimReached']='Память полностью заполнена';
    $ru['InvExt']='Недействительное расширение';
    $ru['FileNoOverw']='Файл не может быть перезаписан';
    $ru['FileOverw']='Файл перезаписан';
    $ru['FileIgnored']='Файл игнорирован';
    $ru['ChkVer']='Поиск обновлений';
    $ru['ChkVerAvailable']=' Доступна новая версия; нажмите здесь, чтобы начать загрузку!';
    $ru['ChkVerNotAvailable']='Не найдено новой версии.';
    ...
    $ru['SelNone']='Отмена';
    $ru['SelInverse']='Обратить выбор';
    $ru['Selected_s']='Выбран(ы)';
    $ru['Total']='Всего';
    $ru['Partition']='Раздел';
