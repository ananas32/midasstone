...
PHFileManager.prototype.formatData = function(data) {
    ...
    if (data.directory != '' && typeof data.parent != 'undefined') {
        str += '<div class="col-md-3 col-sm-4 col-xs-6 text-center padding-item"><div class="item-library thumbnail-out droppable ignore" data-info="parent"><a href="javascript:void(0)" class="thumbnail directory"><i class="fa fa-folder fa-5x"></i></a><label>..</label></div></div>';
    }
    ...
    if (typeof data.images != 'undefined') {
        for (var i = 0; i < data.images.length; i++) {
            ...
            if (data.images[i].type == 'directory') {
                ...
                str += '<div class="item-library thumbnail-out draggable" data-info=\'' + JSON.stringify(data.images[i]) + '\'><a href="javascript:void(0)" class="thumbnail image"><img src="' + data.images[i].thumb + '" alt="' + data.images[i].name + '" title="' + data.images[i].name + '" /></a><label>' + data.images[i].name.substr(0, 20) + '</label></div>'
            }
            str += '</div>';
        }
    }
